...
Advertise.prototype.begin = function() {
    ...
    self._serviceDiscovery = '_services._dns-sd._udp.'+ self._domain;
...
function _probe(self, records)
{
    ...
    var pack        = new Pack( new Buffer( self.bufSize ) ),
        sendLimit   = 3,
        interval    = 250,
        sendCount   = 0;
    ...
    function probe()
    {
        ...
        if (self._repack === true)
        {
            // The message has been changed so we must (re)pack it...
            delete self._repack;

            pack.offset = 0;
            if (! self.msg.pack( pack ))
            {
                // :XXX: Packing error...
        ...
        {
            console.log(">>> Probe #%d (with previous message)", sendCount);
        }

        self.socket.send( pack.buf, pack.begin, pack.offset - pack.begin,
                          Mdns.config.port, Mdns.config.ipv4);
...
function _announce(self, records)
{
    ...
    function announce()
    {
        ...
        {
            console.log(">>> Announce #%d (with previous message)", sendCount);
        }
...
function _responder(self, records)
{
    ...
    function goodbye()
    {
        ...
        var msg = Mdns.Message({
            ...
            pack    = new Pack( new Buffer( self.bufSize ) );
            ...
            response.answer.forEach(function(answer) {
                        ...
                        idex < len; idex++)
                {
                        ...
                        (rec.qtype === answer.type))
                    {
                        ...
                        // Remove this record and indicate repacking necessary
                        self._repack = true;
