...
/** @brief  Write the current Header instance to the given writable buffer.
 *  @param  buf     The buffer into which the Header should be packed;
 *  @param  offset  The starting offset within 'buf';
 *
 *  @return The number of bytes packed (-1 on error).
 */
Header.prototype.pack  = function(buf, offset) {
    offset = offset || 0;
    ...
    if ( (len = msg.packUInt16(self.id, buf, offset)) < 0)
    {
    ...
    if ( (len = msg.packUInt16(u16, buf, offset)) < 0)
    {
        self.error = msg.error;
        return -1;
    }
    ...
    if ( (len = msg.packUInt16(self.qdCount, buf, offset)) < 0)
    {
        self.error = msg.error;
        return -1;
    }
    ...
    if ( (len = msg.packUInt16(self.anCount, buf, offset)) < 0)
    {
        self.error = msg.error;
        return -1;
    }
    ...
    if ( (len = msg.packUInt16(self.nsCount, buf, offset)) < 0)
    {
        self.error = msg.error;
        return -1;
    }
    ...
    if ( (len = msg.packUInt16(self.arCount, buf, offset)) < 0)
    {
        self.error = msg.error;
        return -1;
    }
