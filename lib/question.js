...
function Question(msg, config)
{
    ...
    if (self.qname && (! self.qname.match(/\.$/)))
    {
        self.qname += '.';
    }
    if (typeof self.qtype === 'string')
    {
        self.qtype = Consts.str2type(self.qtype);
    }
    if (typeof self.qclass === 'string')
    {
        self.qclass = Consts.str2class(self.qclass);
    }

    if (self.qclass & 0x8000)
    {
        // mDNS -- unicast responses preferred
        self.unicastResponse  = 0x8000;
        self.qclass          &= 0x7fff;
    }
