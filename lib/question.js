 ...
 */
Question.prototype.unpack   = function(buf, offset) {
    var self    = this,
        nBytes  = buf.length,
        minLen  = 5;
    ...
    self.qtype    = msg.unpackUInt16(buf, offset);    offset += 2;
    self.qclass   = msg.unpackUInt16(buf, offset);    offset += 2;
 ...
 */
Question.prototype.pack  = function(buf, offset) {
    offset = offset || 0;
    ...
    if ( (len = msg.packName( self.qname,    buf, offset )) < 0) { return -1; }
    offset += len;

    if ( (len = msg.packUInt16( self.qtype,  buf, offset )) < 0) { return -1; }
    offset += len;

    if ( (len = msg.packUInt16( self.qclass, buf, offset )) < 0) { return -1; }
    offset += len;
