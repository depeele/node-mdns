...
function RR(msg, config)
{
    ...
    Object.defineProperties(self, {
        ...
        'name': {
            get:        function()      { return self._data.name; },
            set:        function(val)   {
                self._data.name = val;
            },
            enumerable: true
        ...
        'type': {
            get:        function()      { return self._data.type; },
            set:        function(val)   {
        ...
        'class':    {
            get:        function()      { return self._data.class; },
            set:        function(val)   {
                if (typeof val === 'string')
                {
                ...
                if (val & 0x8000)
                {
                    /* mDNS -- this record is a member of a unique RRSet, and
                     *         the entire RRSet has been sent together
                     *         (cache-flush)
                     */
                    self.cacheFlush = true;
                    val            &= 0x7fff;
                }
                else
                {
                    delete self.cacheFlush;
                }

                self._data.class = val;
            },
            enumerable:     true
        },
        'ttl':  {
            get:        function()      { return self._data.ttl; },
            set:        function(val)   {
        ...
        'rdata':    {
            get:        function()      { return self._data.rdata; },
            set:        function(val)   {
                if (! (val instanceof RData))
                {
                    // Create a new RData instance
                    val = new RData(self, val);
                }
                else
                {
...
RR.prototype.toString  = function(prefix, lineLen) {
    ...
    str += Util.format("%s: type:%d (%s), class:%d (%s)%s, ttl:%d\n",
                       self.name,
...
RR.prototype.pack  = function(pack) {
    ...
    var cls     = self.class | (self.cacheFlush === true ? 0x8000 : 0);

    if ( (pack.domainName( self.name )   === null) ||
         (pack.uint16(     self.type )   === null) ||
         (pack.uint16(          cls  )   === null) ||
