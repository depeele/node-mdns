 ...
 */
RR.prototype.unpack   = function(buf, offset) {
    var self    = this,
        nBytes  = buf.length,
        minLen  = 11;
    ...
    self.type     = msg.unpackUInt16(buf, offset);    offset += 2;
    self.typeStr  = RData.prototype.type2str(self.type);
    self.class    = msg.unpackUInt16(buf, offset);    offset += 2;
    self.ttl      = msg.unpackUInt32(buf, offset);    offset += 4;

    self.rdata    = new RData( self, buf, offset );
    if (self.rdata.error)
    {
 ...
 */
RR.prototype.pack  = function(buf, offset) {
    offset = offset || 0;
    ...
    if ( (len = msg.packName( self.name,     buf, offset )) < 0) { return -1; }
    offset += len;

    if ( (len = msg.packUInt16( self.type,   buf, offset )) < 0) { return -1; }
    offset += len;

    if ( (len = msg.packUInt16( self.class,  buf, offset )) < 0) { return -1; }
    offset += len;

    if ( (len = msg.packUInt32( self.ttl,    buf, offset )) < 0) { return -1; }
    offset += len;

    if ( (len = self.rdata.pack(             buf, offset )) < 0) { return -1; }
    offset += len;
