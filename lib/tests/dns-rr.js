...
var unpack  = new Unpack( new Buffer( data ),        offset ),
    pack    = new Pack(   new Buffer( data.length ), offset );

for (var idex = 0; idex < nRecs; idex++)
{
    console.log("# %d @ %s ------------------------------------------------",
                idex+1, Utils.buf2hex(offset, 2));
    ...
    console.log("Unpack RR from %d bytes of raw data:\n%s",
                rr.consumed,
                Utils.buf2hex(unpack.buf, offset, rr.consumed, 16));
    ...
    console.log("Packed RR (%d bytes):\n%s",
                rr.produced,
                Utils.buf2hex(pack.buf, offset, rr.produced, 16));
