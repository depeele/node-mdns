...
console.log("--------------------------------------------------------------");
var name    = 'test.local.',
    message = Mdns.Message({ header: {id:0x1234, qr:1, ra:1} });

message.addQuestion( Mdns.Question({qname:name, qtype:1, qclass:1}) );

message.addAnswer( Mdns.RR({
        name:name, type:'SOA', 'class':'IN', ttl:1,
        rdata: {
