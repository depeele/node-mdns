 ...
 *  @param  len     If provided, the number of bytes to pack [ value.length ];
 *
 *  @return The number of bytes packed (null on error).
 */
Pack.prototype.data = function(value, len) {
    var self    = this;
    ...
    value.copy( self.buf, self.offset, 0, produce);
...
Pack.prototype.AAAA = function(value) {
    ...
    for (var idex = 0, len = parts.length; idex < len; idex++)
    {
        if (parts[idex] === '')
        {
            // zero fill
            for (var jdex = (8 - len + 1); jdex > 0; jdex--)
            {
                if ( (self.uint16( 0 )) === null)
                {
                    return null;
                }
            }
        ...
        // Parse this segment (2-bytes)
        uint16 = parseInt( parts[idex], 16);
        if ( (self.uint16( uint16 )) === null)
        {
            return null;
        }
    }
...
Pack.prototype.domainName   = function(value, noCompression) {
    ...
    while (parts.length > 0)
    {
        ...
        if ((noCompression !== true) && self.labelMap[partial])
        {
             ...
             */
            ptr = ptrId | (self.labelMap[partial] & ptrMask);
            len = self.uint16(ptr);
            if (len < 0)
            {
                if (! self.error)
