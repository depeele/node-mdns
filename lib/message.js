 ...
 *                          question    An array of Question instances or
 ...
 *  @param  qName   The domain-name of this question (string);
...
Message.prototype.addQuestion = function(qName, qType, qClass) {
    ...
    question.qtype  = intType;
    question.qclass = intClass;
 ...
 *  @param  rName   The domain-name of this record (string);
...
Message.prototype.pack  = function(buf, offset) {
    ...
    // Question
    for (idex=0;(idex < self.header.qdCount) && (! self.isTruncated());idex++)
    {
    ...
    for (idex=0;(idex < self.header.anCount) && (! self.isTruncated());idex++)
    {
        ...
        if (! rr.pack( pack ))
        {
            if (rr.error instanceof Pack.TruncError)
            {
                self.truncate();
            }
            self.error = rr.error;
    ...
    // Authorities
    for (idex=0;(idex < self.header.nsCount) && (! self.isTruncated());idex++)
    {
        rr = self.authority[ idex ];
        if (! rr)   { continue; }
    ...
    // Additionals
    for (idex=0;(idex < self.header.arCount) && (! self.isTruncated());idex++)
    {
        rr = self.additional[ idex ];
        if (! rr)   { continue; }
